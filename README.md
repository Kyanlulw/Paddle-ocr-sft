...
Use `mixed30k.json` as the training annotation source and provide separate
image roots for the two underlying datasets:

```bash
bash train_mixed30k.sh
```

Expected `mixed30k.json` row format:

```json
{"image":"14288.jpg","question":"...","answer":"...","_source":"dataset1"}
{"image":"000000004169.jpg","question":"...","answer":"...","_source":"dataset2"}
```

Notes:
- `_source` drives image routing: `dataset1 -> dataset1_image_root`, `dataset2 -> dataset2_image_root`
- The line `15000/15001` source boundary is checked as a sanity guard
- `openvivqa_train_v2.json` is optional and is only used to validate or recover dataset2 filenames
- Evaluation is optional for `mixed30k`; if you do not provide `--eval_annotation_path`, set `--eval_strategy no`
...
**Switch dataset backend:**
```bash
python sft_paddleocr_vl.py \
    --dataset_backend mixed30k \
    --train_annotation_path ./mixed30k.json \
    --dataset1_image_root /path/to/dataset1/images \
    --dataset2_image_root /path/to/dataset2/images \
    --dataset2_metadata_path /path/to/openvivqa_train_v2.json \
    --eval_strategy no
```

New dataset arguments:
- `--dataset_backend`: `manga109` or `mixed30k`
- `--train_annotation_path`: required for `mixed30k`
- `--eval_annotation_path`: optional for `mixed30k`
- `--dataset1_image_root`: required for `mixed30k`
- `--dataset2_image_root`: required for `mixed30k`
- `--dataset2_metadata_path`: optional OpenViVQA metadata sidecar
